      ...
      body: SafeArea(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              customText(widget.productsModel.table![widget.index].name.toString(), 50, AppColors.text1Color, FontWeight.bold),
              customImage(widget.productsModel.table![widget.index].image.toString(), widget.height * 0.5, widget.width),
              Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 10),
                  child:Column(
                    children: [
                      customText("Description", 20, AppColors.text1Color, FontWeight.bold),
                      Text(widget.productsModel.table![widget.index].description.toString(),style: TextStyle(color: AppColors.text3Color,fontSize: 15,fontWeight: FontWeight.normal),textAlign: TextAlign.justify,)
                    ],
                  )
              ),
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  customText("RS ${widget.productsModel.table![widget.index].price.toString()}", 30, AppColors.text2Color, FontWeight.bold),
                  InkWell(
                    onTap: (){
                      if(quantity >=1){
                        cartController.addItem(widget.productsModel.table![widget.index].name.toString(), widget.productsModel.table![widget.index].image.toString(), double.parse(widget.productsModel.table![widget.index].price.toString()), widget.productsModel.table![widget.index].description.toString(), quantity);
