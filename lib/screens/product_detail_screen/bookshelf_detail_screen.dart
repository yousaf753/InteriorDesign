...
class BookShelfDetailScreen extends StatefulWidget {
  ...
  @override
  _BookShelfDetailScreenState createState() => _BookShelfDetailScreenState();
}
      ...
      body: SafeArea(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              customText(widget.productsModel.bookshelf![widget.index].name.toString(), 50, AppColors.text1Color, FontWeight.bold),
              customImage(widget.productsModel.bookshelf![widget.index].image.toString(), widget.height * 0.5, widget.width),
              Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 10),
                  child:Column(
                    children: [
                      customText("Description", 20, AppColors.text1Color, FontWeight.bold),
                      Text(widget.productsModel.bookshelf![widget.index].description.toString(),style: TextStyle(color: AppColors.text3Color,fontSize: 15,fontWeight: FontWeight.normal),textAlign: TextAlign.justify,)
                    ],
                  )
              ),
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  customText("RS ${widget.productsModel.bookshelf![widget.index].price.toString()}", 30, AppColors.text2Color, FontWeight.bold),
                  InkWell(
                    onTap: (){
                      if(quantity >=1){
                        cartController.addItem(widget.productsModel.bookshelf![widget.index].name.toString(), widget.productsModel.bookshelf![widget.index].image.toString(), double.parse(widget.productsModel.bookshelf![widget.index].price.toString()), widget.productsModel.bookshelf![widget.index].description.toString(), quantity);
